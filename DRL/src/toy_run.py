...
def run_epoch(envt, central_agent, value_function, requests, request_generator, agents_predefined, is_training = False):
	...
	for t in range(ts):
		...
		feasible_actions = central_agent.get_feasible_actions(deepcopy(agents), deepcopy(current_orders))

		# Get other external info to add to post-decision state
		num_robots_charging, avg_robot_battery_percentage, avg_robot_capacity, avg_human_capacity, num_human_only_orders, num_both_orders = central_agent.get_external_infor(deepcopy(agents), deepcopy(current_orders))
		...
		matchings, labeled_assignments, available_agent_actions = value_function.make_matchings(deepcopy(agents), feasible_actions, action_values, humans_first, deepcopy(current_orders))
...
if __name__ == '__main__':
	...
	parser.add_argument('-train_days', '--train_days', type=int, default=100)
	...
	for _ in tqdm(range(args.train_days)):
		run_epoch(envt, central_agent, value_function, deepcopy(toy_orders), None, deepcopy(toy_agents), True)
